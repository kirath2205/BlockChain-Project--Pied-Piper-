...
contract Approval {
    ...
    function approve_proposals(uint[] memory proposalIDs) public returns (uint) {
        ...
        uint memory len = proposalIDs.length;
        ...
        if (approval_count == gt.getCouncilCount()) {
            uint[] final_proposals = get_final_proposals();
    ...
    function get_final_proposals() private returns (uint[]) {
        ...
        uint[] memory current_approvals = approvals[g.get_current_epoch()];
        ...
        // }
        for (uint i = 0; i<proposal_count; i++){
            if (current_approvals[i] > min_approvals) {
    ...
    function get_member_approval_status() public view returns (uint8) {
        return member_status[address];
    }
    
    function get_proposal_approval_count(uint memory proposalID) public view returns (uint memory) {
